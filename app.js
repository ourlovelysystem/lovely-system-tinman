(()=>{
  ...
  $('modeCopy').textContent=mode==='appeal'?'Record an appeal. Say what happened in your own voice.':'Record a message in your own voice.';
  ...
  $('stopButton').onclick=()=>{if(recorder?.state==='recording'){recorder.stop();clearInterval(timer);$('stopButton').disabled=true;$('recordButton').disabled=false}};
  ...
  $('submitButton').onclick=async()=>{
    ...
    try{
      ...
      const init=await api('/recordings/init',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({self_id:selfId,message_type:mode,content_type:blob.type||'audio/webm',duration_seconds:duration})});
  ...
  async function loadRecordings(){
    ...
    try{
      ...
      $('recordings').innerHTML=data.items.length?data.items.map(item=>`<article class="recording"><div class="meta"><div><div class="who">${escapeHtml(item.self_id)}</div><div class="details">${escapeHtml(item.message_type)} · ${format(item.duration_seconds)}</div></div><time class="when">${new Date(item.created_at).toLocaleString()}</time></div><audio controls preload="none" src="${escapeHtml(item.play_url)}"></audio></article>`).join(''):'<p class="empty">No recordings have been submitted.</p>';
    }catch(error){$('recordings').innerHTML=`<p class="empty">Recording browser unavailable: ${escapeHtml(error.message)}</p>`}
  }
  $('refreshButton').onclick=loadRecordings;loadRecordings();
})();
